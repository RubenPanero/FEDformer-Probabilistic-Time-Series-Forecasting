...
def test_dataset_loading(sample_csv):
    config = FEDformerConfig(target_features=['Close'], file_path=sample_csv, date_column='date', seq_len=10, pred_len=4, label_len=5)
    dataset = TimeSeriesDataset(config=config, flag='train')
    assert len(dataset) > 0


def test_dataset_scaling(sample_csv):
    config = FEDformerConfig(target_features=['Close'], file_path=sample_csv, date_column='date', seq_len=10, pred_len=4, label_len=5)
    dataset = TimeSeriesDataset(config=config, flag='train')
...
def test_dataset_inverse_scaling(sample_csv):
    config = FEDformerConfig(target_features=['Close'], file_path=sample_csv, date_column='date', seq_len=10, pred_len=4, label_len=5)
    dataset = TimeSeriesDataset(config=config, flag='train')
    original_data = dataset.scaler.inverse_transform(dataset.data_x)
    assert np.allclose(original_data.mean(), pd.read_csv(sample_csv).drop('date', axis=1).mean().mean(), atol=100)


def test_dataset_item(sample_csv):
    config = FEDformerConfig(target_features=['Close'], file_path=sample_csv, seq_len=10, pred_len=4, label_len=5, date_column='date')
    dataset = TimeSeriesDataset(config=config, flag='train')
    item = dataset[0]
...
def test_dataset_sequence_creation(sample_csv):
    config = FEDformerConfig(target_features=['Close'], file_path=sample_csv, seq_len=10, pred_len=4, label_len=5, date_column='date')
    dataset = TimeSeriesDataset(config=config, flag='train')
    item = dataset[0]
    # The last `label_len` values of the encoder input should be the same as the first `label_len` values of the decoder input
    assert np.allclose(item['x_enc'][-config.label_len:], item['x_dec'][:config.label_len])
